...
if (issue.customFields."Existing Bug".value == null){
    def _rJson
    def res = new ZdClient(httpClient, debug)
        .http(
                ...
                [:],
                "{\"organization\": {\"name\": \"${replica.Name}\", \"organization_fields\": {\"test_copy\": \"${replica.Web}\"}}}",
                ["Accept": ["application/json"], "Content-type" : ["application/json"]]
        ) 
        { 
            response ->
            if (response.code >= 400) {
                debug.error("${response.properties}")
    ...
    issue.customFields."Existing Bug".value = _rJson.organization.id
}
else{
    def _rJson
    def res = new ZdClient(httpClient, debug)
        .http(
                "PUT",
                "/api/v2/organizations/${issue.customFields.'Existing Bug'.value}",
                [:],
                "{\"organization\": {\"name\": \"${replica.Name}\", \"organization_fields\": {\"test_copy\": \"${replica.Web}\"}}}",
                ["Accept": ["application/json"], "Content-type" : ["application/json"]]
        ) 
